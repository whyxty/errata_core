...
def render(cfg: dict):
    ...
    with st.expander("О задаче", expanded=False):
        st.markdown("""
**Цель:** предсказать, насколько вырастет пористость песчаного коллектора после
глинокислотной обработки, когда прямых лабораторных замеров kmg нет — только данные
о количестве растворённых кислотами карбонатов и глин.

**Как решается:** по массовой доле растворённого глинокислотой вещества DGg и плотностям
породы пересчитывается объёмная доля: DVg = ρп·DGg/ρск (В.82) — это и есть прирост
пористости в процентах. Дальше: пористость после ГКР mg = ms + DVg (В.83), коэффициент
роста kmg = mg/ms (В.84) и суммарный рост после обеих обработок kmsg = kms·kmg (В.85).

**Результат:** ожидаемая пористость mg и коэффициенты kmg, kmsg — те самые, что в задаче
«Целесообразность ГКО» проверяются по критериям В.4–В.5 (порог 1,1).
""")
