"""Задача В.3 — Расход жидкости и давление во время нагнетания
...
def render(cfg: dict):
    ...
    with st.expander("О задаче", expanded=False):
        st.markdown("""
**Суть:** определить, с каким расходом и при каком давлении можно безопасно закачивать
кислотный раствор, чтобы не порвать пласт гидроразрывом и не повредить колонну.

**Как решается:** на скважине проводят пробное нагнетание с расходом q0 и снимают кривую
приёмистости pу = f(t) — давление на устье растёт и выходит на полку. Эта полка —
квазиустойчивое давление pк. Дальше проверки:

- расход не ниже минимального: qк ≥ qпр (В.10);
- давление не выше опрессовочного: pк ≤ pопр — иначе нельзя качать без пакера (В.15);
- гидростатика pгст = ρgH·10⁻⁶ (В.19) и сравнение градиентов на забое:
  grad pк = (pгст + pк)/(0,01·H) против градиента гидроразрыва grad pгрп
  (из исследований или регионального, В.17–В.20);
- условие безопасности: grad pк < grad pгрп (В.16).

**Результат:** допустимые расход qк и давление нагнетания + заключение,
что гидроразрыв при закачке не ожидается.
""")
