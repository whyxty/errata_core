...
# Пример В.16.1 (табл. В.25). ks, kg растут с объёмом обработки (иллюстративно;
# точные значения берут из табл. В.9/В.23/В.13 — поля редактируемые).
EXAMPLE_V16 = {
    ...
    "variants": [
        ...
        {"Vскр, м³": 6, "Vгкр, м³": 6, "rз.рs, м": 0.76, "rз.рg, м": 0.61, "ks, мкм²": 0.082, "kg, мкм²": 0.182, "Zко, руб": 4000},
        {"Vскр, м³": 9, "Vгкр, м³": 9, "rз.рs, м": 0.92, "rз.рg, м": 0.74, "ks, мкм²": 0.089, "kg, мкм²": 0.193, "Zко, руб": 5000},
    ],
}
...
def render(cfg: dict):
    ...
    with st.expander("О задаче", expanded=False):
        st.markdown("""
**Цель:** итоговая проверка деньгами — сколько дополнительной нефти даст обработка
и окупится ли она. Сравниваются варианты с разными объёмами закачки кислоты.

**Как решается:** после обработки вокруг скважины образуются две зоны улучшенной
проницаемости — СКР (радиус rз.рs, проницаемость ks) и ГКР (rз.рg, kg). Приток через
такой «слоёный» радиальный пласт даёт коэффициент роста дебита Ag (В.86). Новый дебит
с учётом охвата разреза: Qg = Ag·εот·Qф + (1−εот)·Qф (В.89). Дополнительная добыча
нефти за период Tн с поправкой на обводнённость W0: ΔQн = (Ag−1)·εот·Qф·Tн·ρн·(100−W0)/100
(В.90). Экономика: Эн = (Цн − Сн)·ΔQн − Zко (В.91) — выручка минус себестоимость минус
затраты на обработку.

**Результат:** для каждого варианта объёма кислоты — рост дебита Ag, новый дебит Qg,
дополнительная нефть ΔQн и экономический эффект Эн; выбирается оптимальный объём обработки.
""")
