...
def render(cfg: dict):
    ...
    if btn_col.button("Пример ГКО", key="ex_v11_gko", type="secondary", use_container_width=True):
        _load_example(EXAMPLE_GKO); st.rerun()

    with st.expander("О задаче", expanded=False):
        st.markdown("""
**Цель:** прежде чем закачивать кислоту — доказать, что обработка даст эффект.
...
- **Резерв продуктивности.** ОП = Кф/Кпот и ОД = Qф/Qож показывают, насколько скважина
  недорабатывает относительно потенциала пласта. При ОП ≈ 1 скважина работает на полную,
  и кислотная обработка прироста не даст.
- **Доставка кислоты.** Приёмистость (В.10), поглощающие пласты (В.3), относительная
  гидропроводность (В.13) — раствор должен поглотиться именно загрязнёнными интервалами,
  а не уйти в высокопроницаемые пласты.
- **Химическая эффективность.** Глинистость (В.12) ограничивается, чтобы реакция HF
  не дала нерастворимый осадок; лабораторные kms, kmg (В.4, В.5) подтверждают, что кислота
  реально улучшает коллектор; карбонатность Cк (В.7) определяет тип обработки:
  ≥ 3 % — СКО→ГКО, < 3 % — только ГКО.
- **Самоочистка после обработки.** Энергетика kэнр = pпл/pгст (В.14) — пластового давления
  должно хватить, чтобы вынести продукты реакции из ПЗП.
- **Схема обработки.** Коэффициент охвата kв.о = hпга/hэф (В.8) определяет, обрабатывать
  весь разрез сразу или поинтервально.
...
""")
    ...
    with st.expander("Формулы", expanded=False):
        st.latex(r"\mathrm{ОП} = K_\phi / K_{\text{пот}} < 1 \quad\text{(В.1)}")
        st.latex(r"\mathrm{ОД} = Q_\phi / Q_{\text{ож}} < 1, \qquad "
                 r"Q_{\text{ож}} = \sum q_{\text{уд}}(m_0)\,h_{\text{эф}} \quad\text{(В.2, В.9)}")
        st.latex(r"m_0 > m_{\text{гр}} \quad\text{(В.2)}")
        st.latex(r"h_{\text{пга}} > h_{\text{пр}} \quad\text{(В.3)}")
        st.latex(r"h_{\text{эф.сум}} \ge h_{\text{пр}} \quad\text{(В.11)}")
        st.latex(r"q \ge q_{\text{пр}} \quad\text{(В.10)}")
        st.latex(r"C_{\text{гл}} \le C_{\text{г.пр}} \quad\text{(В.12)}")
        st.latex(r"\varepsilon_{\text{от}} = \varepsilon_{\text{обр}} / \varepsilon_{\text{скв}} > 0{,}5 \quad\text{(В.13)}")
        st.latex(r"k_{\text{энр}} = p_{\text{пл}} / (\rho g H \cdot 10^{-6}) > 0{,}7 \quad\text{(В.14)}")
        st.latex(r"k_{ms} \ge k_{s\text{пр}}, \qquad k_{mg} \ge k_{g\text{пр}} \quad\text{(В.4, В.5)}")
        st.latex(r"C_{\text{к}} \ge C_{\text{к.пр}} \rightarrow \text{СКО→ГКО; иначе — только ГКО} \quad\text{(В.7)}")
        st.latex(r"k_{\text{в.о}} = h_{\text{пга}} / h_{\text{эф.сум}} \quad\text{(В.8)}")
        st.markdown("*Kф, Kпот — фактическая и потенциальная продуктивность; Qф, Qож — фактический "
                    "и ожидаемый дебит; m₀, mгр — пористость пласта и граничная (8 %); hпга — толщина "
                    "поглощающих пластов; hпр = 5 м; qпр = 24 м³/сут; Cг.пр = 10 %; Cк.пр = 3 %; "
                    "ksпр = kgпр = 1,1; εобр — Σ hэф·k₀ по обрабатываемым пластам.*")
