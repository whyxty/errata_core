...
_COLS = ["Интервал", "hэф, м", "m0, %", "k0, мкм²", "Обрабатываемый"]

_DEF_LAYERS = [
    {"Интервал": "", "hэф, м": 0.0, "m0, %": 0.0, "k0, мкм²": 0.0, "Обрабатываемый": False},
]

# ── Пример В.1 (полная информация — продуктивность ОП) ──
EXAMPLE_B1 = {
    "Kf": 16.0, "Kpot": 51.0, "Qf": 0.0, "q": 0.0,
    "p_pl": 0.0, "H": 0.0, "C_k": 4.0, "C_gl": 5.0,
    "kms": 1.2, "kmg": 1.1, "h_pga": 17.0,
    "layers": [
        {"Интервал": "2733–2740", "hэф, м": 7.0,  "m0, %": 12.0, "k0, мкм²": 0.0, "Обрабатываемый": True},
        {"Интервал": "2756–2768", "hэф, м": 12.0, "m0, %": 9.8,  "k0, мкм²": 0.0, "Обрабатываемый": False},
        {"Интервал": "2785–2795", "hэф, м": 10.0, "m0, %": 13.0, "k0, мкм²": 0.0, "Обрабатываемый": True},
        {"Интервал": "2808–2820", "hэф, м": 12.0, "m0, %": 10.5, "k0, мкм²": 0.0, "Обрабатываемый": False},
        {"Интервал": "2823–2851", "hэф, м": 18.0, "m0, %": 9.6,  "k0, мкм²": 0.0, "Обрабатываемый": False},
    ],
}

# ── Пример В.2 (ограниченная информация — дебит ОД) ──
EXAMPLE_B2 = {
    "Kf": 0.0, "Kpot": 0.0, "Qf": 86.6, "q": 150.0,
    "p_pl": 25.0, "H": 2800.0, "C_k": 4.0, "C_gl": 5.0,
    "kms": 1.2, "kmg": 1.1, "h_pga": 27.4,
    "layers": [
        {"Интервал": "7–9 %",   "hэф, м": 13.2, "m0, %": 8.0,  "k0, мкм²": 0.0007, "Обрабатываемый": False},
        {"Интервал": "9–11 %",  "hэф, м": 27.2, "m0, %": 9.9,  "k0, мкм²": 0.0030, "Обрабатываемый": False},
        {"Интервал": "11–13 %", "hэф, м": 14.1, "m0, %": 12.0, "k0, мкм²": 0.0213, "Обрабатываемый": False},
        {"Интервал": "13–15 %", "hэф, м": 27.4, "m0, %": 13.9, "k0, мкм²": 0.0446, "Обрабатываемый": True},
    ],
}
...
def render(cfg: dict):
    ...
    title_col, b1_col, b2_col = st.columns([4, 1, 1])
    title_col.subheader("Проверка целесообразности выбора скважины для ГКО")
    if b1_col.button("Пример В.1", key="ex_v11_b1", type="secondary", use_container_width=True):
        _load_example(EXAMPLE_B1); st.rerun()
    if b2_col.button("Пример В.2", key="ex_v11_b2", type="secondary", use_container_width=True):
        _load_example(EXAMPLE_B2); st.rerun()

    st.caption("Объединяет методики В.1 (полная информация — критерий ОП) и "
               "В.2 (ограниченная информация — критерий ОД). Расчёт — в реальном времени.")
...
""")
    ...
    for c in _COLS:
        if c not in df.columns:
            df[c] = False if c == "Обрабатываемый" else (0.0 if c != "Интервал" else "")
    df = df[_COLS]
    edited = st.data_editor(
        df, num_rows="dynamic", use_container_width=True, key="v11_editor",
        column_config={
            "Интервал":      st.column_config.TextColumn("Интервал", width="medium"),
