...
def render(cfg: dict):
    ...
    with st.expander("О задаче", expanded=False):
        st.markdown("""
**Суть:** кислота сама по себе не дойдёт до пласта — её нужно протолкнуть.
Задача считает объёмы двух вспомогательных жидкостей.

**Как решается:**

- **Вытесняющая жидкость** — выталкивает кислотный раствор из ствола в пласт:
  Vвтс = 1,2·Vз.рs по объёму зоны растворения (В.56), а если он неизвестен —
  оценка с запасом Vвтс = 0,3·Vкс (В.57).
- **Продавочная жидкость** — заполняет НКТ и затрубное пространство:
  Vпрд = 0,785·[dв²·Hн.о + (Dк² − dвн²)·(Hн.о − Hв.о)] (В.58) — геометрический объём
  труб (внутренний диаметр НКТ dв на всю глубину) плюс кольцевого зазора между
  колонной Dк и НКТ dвн в интервале обработки.

**Результат:** объёмы Vвтс и Vпрд в м³ — сколько жидкости готовить на операцию.
""")
